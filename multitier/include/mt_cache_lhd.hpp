...
/**
 * Implements a multi-tiered LHD cache.
 */
class MTLHDCache : public MTBaseCache {
private:
    static constexpr double kExplorerBudgetFraction = 0.01;
...
public:
    MTLHDCache(TierConfig t, int index, const bool penalize_insertions, const HashType hash_type) :
        ...
        for (size_t idx = 0; idx < kMaxNumCacheSets; idx++) {
            cache_sets_.push_back(new LHDCacheSet<MTLHDCache>(
