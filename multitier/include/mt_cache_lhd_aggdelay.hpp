...
public:
    Rand_(size_t seed = 0) : state(seed) {}
    inline size_t next() {
...
private:
    ...
    std::unordered_map<std::string, FlowMetadata_LHD> records_; // Dict mapping flow IDs to records
...
public:
    LHDAggregateDelayCacheSet(const size_t num_entries, T& cache) :
        ...
        // The update was unsuccessful, create a new entry to insert
            ...
            if (entries_.size() == getNumEntries()) {
                ...
                for (const auto& pair : entries_) {
                    ...
                    const double candidate_cost = kCacheImpl.getHitDensityPayoff(
                ...
                CacheEntry evicted_entry = entries_.at(flow_id_to_evict);
 ...
 */
class MTLHDAggregateDelayCache : public MTBaseCache {
private:
    static constexpr double kExplorerBudgetFraction = 0.01;
    ...
    std::vector<ClassMetadata_LHD> classes_; // List of object classes
    std::unordered_map<std::string, FlowState> states_; // Dict mapping flow IDs to states

public:
    MTLHDAggregateDelayCache(TierConfig t, int index, const bool penalize_insertions, const HashType hash_type):
                MTBaseCache(t, index, penalize_insertions, hash_type), rand(12345) {

        next_reconfiguration_ = kAccessesPerReconfiguration;
        for (size_t i = 0; i < kNumClassesTotal; i++) {
            classes_.push_back(ClassMetadata_LHD());
        ...
        for (size_t idx = 0; idx < kMaxNumCacheSets; idx++) {
            cache_sets_.push_back(new LHDAggregateDelayCacheSet
                <MTLHDAggregateDelayCache>(kCacheSetAssociativity, *this));
        }
    }
    virtual ~MTLHDAggregateDelayCache() {}

    /**
     * Returns the canonical cache name.
     */
    virtual std::string name() const override { return "LHDAggregateDelayCache"; }
     ...
     */
    inline size_t getClassId(const FlowMetadata_LHD& data) const {
     ...
     */
    inline ClassMetadata_LHD& getClass(const FlowMetadata_LHD& data) {
     ...
     */
    inline size_t getAge(const FlowMetadata_LHD& data) {
    ...
    inline double getHitDensityPayoff(const std::string& id,
        ...
        const ClassMetadata_LHD& cl = getClass(data);
    ...
    void updateClass(ClassMetadata_LHD& cl) {
        cl.total_hits = 0;
    ...
    void update(std::unordered_map<std::string, FlowMetadata_LHD>& records,
        ...
        if (insert) {
            FlowMetadata_LHD& data = records[id];
            ...
            FlowMetadata_LHD& data = iter->second;
            ClassMetadata_LHD& cl = getClass(data);
    ...
    void replaced(std::unordered_map<std::string, FlowMetadata_LHD>& records,
                  const std::string& id, int64_t& explorer_budget) {
        auto iter = records.find(id);
        assert(iter != records.end());

        // Record stats before removing item
        const FlowMetadata_LHD& data = iter->second;
        ClassMetadata_LHD& cl = getClass(data);
        auto age = getAge(data);
        cl.evictions[age] += 1;
